...
/** To perform queries and updates on multiple threads, you'll want to use @c LVKFMDatabaseQueue .
 *
 * Using a single instance of @c LVKFMDatabase from multiple threads at once is a bad idea.  It has always been OK to make a @c LVKFMDatabase  object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.
 *
 * Instead, use @c LVKFMDatabaseQueue . Here's how to use it:
 *
 * First, make your queue.
 *
 * @code
 * LVKFMDatabaseQueue *queue = [LVKFMDatabaseQueue databaseQueueWithPath:aPath];
 * @endcode
 *
 * Then use it like so:
 *
 * @code
 * [queue inDatabase:^(LVKFMDatabase *db) {
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 *
 *  LVKFMResultSet *rs = [db executeQuery:@"select * from foo"];
 *  while ([rs next]) {
 *      //…
 *  }
 * }];
 ...
 *
 * @code
 * [queue inTransaction:^(LVKFMDatabase *db, BOOL *rollback) {
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
 *  [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 *
 ...
 * @endcode
 *
 * @c LVKFMDatabaseQueue will run the blocks on a serialized queue (hence the name of the class).  So if you call @c LVKFMDatabaseQueue 's methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.
 *
 * @warning Do not instantiate a single @c LVKFMDatabase  object and use it across multiple threads. Use @c LVKFMDatabaseQueue  instead.
 *
 * @warning The calls to @c LVKFMDatabaseQueue 's methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.
 *
 * @sa LVKFMDatabase
 *
 */

@interface LVKFMDatabaseQueue : NSObject
 ...
 *
 * @return The @c LVKFMDatabaseQueue  object. @c nil  on error.
 */
...
// If you need to nest, use LVKFMDatabase's startSavePointWithName:error: instead.
