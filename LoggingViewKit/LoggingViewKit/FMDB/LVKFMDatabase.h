...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define LVKFMDBAutorelease(__v)          ([__v autorelease]);
    #define LVKFMDBReturnAutoreleased LVKFMDBAutorelease

    #define LVKFMDBRetain(__v)               ([__v retain]);
    #define LVKFMDBReturnRetained     LVKFMDBRetain

    #define LVKFMDBRelease(__v)              ([__v release]);

    #define LVKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
// -fobjc-arc
    #define LVKFMDBAutorelease(__v)
    #define LVKFMDBReturnAutoreleased(__v)   (__v)

    #define LVKFMDBRetain(__v)
    #define LVKFMDBReturnRetained(__v)       (__v)

    #define LVKFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define LVKFMDBDispatchQueueRelease(__v)
    #else
        #define LVKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef int (^LVKFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 * Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, LVKFMDBCheckpointMode) {
    LVKFMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    LVKFMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    LVKFMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    LVKFMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
 ...
 * Usage
 *
 * The three main classes in LVKFMDB are:
 *
 * - @c LVKFMDatabase - Represents a single SQLite database.  Used for executing SQL statements.
 *
 * - @c LVKFMResultSet - Represents the results of executing a query on an @c LVKFMDatabase .
 *
 * - @c LVKFMDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.
 *
 * See also
 *
 * - @c LVKFMDatabasePool - A pool of @c LVKFMDatabase objects
 *
 * - @c LVKFMStatement - A wrapper for @c sqlite_stmt
 *
 * External links
 *
 ...
 *
 * @warning Do not instantiate a single @c LVKFMDatabase  object and use it across multiple threads. Instead, use @c LVKFMDatabaseQueue .
 *
 */

@interface LVKFMDatabase : NSObject
...
/** Create a @c LVKFMDatabase  object.
 *
 * An @c LVKFMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:
 *
 * 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 *
 * 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c LVKFMDatabase  connection is closed.
 *
 * 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c LVKFMDatabase  connection is closed.
 *
 ...
 * LVKFMDatabase *db = [LVKFMDatabase databaseWithPath:fileURL.path];
 * @endcode
 *
 ...
 *
 * @return @c LVKFMDatabase  object if successful; @c nil  if failure.
 *
 */
 ...
 *
 * 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c LVKFMDatabase  connection is closed.
 *
 * For example, to open a database in the app's “Application Support” directory:
 *
 * @code
 * NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 * NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 * LVKFMDatabase *db = [LVKFMDatabase databaseWithURL:fileURL];
 * @endcode
 *
 * (For more information on temporary and in-memory databases, read the sqlite documentation on the subject: [https://sqlite.org/inmemorydb.html](https://sqlite.org/inmemorydb.html))
 *
...
/** Initialize a @c LVKFMDatabase  object.
 *
 * An @c LVKFMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:
 *
 * 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 *
 * 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c LVKFMDatabase  connection is closed.
 *
 * 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c LVKFMDatabase  connection is closed.
 *
 * For example, to open a database in the app's “Application Support” directory:
 ...
 * NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 * NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 * LVKFMDatabase *db = [[LVKFMDatabase alloc] initWithPath:fileURL.path];
 * @endcode
 *
 *
 * (For more information on temporary and in-memory databases, read the sqlite documentation on the subject: [https://sqlite.org/inmemorydb.html](https://sqlite.org/inmemorydb.html))
...
/** Initialize a @c LVKFMDatabase  object.
 *
 * An @c LVKFMDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 *
 * 1. A file system URL.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 *
 * 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c LVKFMDatabase  connection is closed.
 ...
 * NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 * NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 * LVKFMDatabase *db = [[LVKFMDatabase alloc] initWithURL:fileURL];
 * @endcode
 *
 * (For more information on temporary and in-memory databases, read the sqlite documentation on the subject: [https://sqlite.org/inmemorydb.html](https://sqlite.org/inmemorydb.html))
 *
...
- (BOOL) executeStatements :(NSString *) sql withResultBlock :(__attribute__((noescape)) LVKFMDBExecuteStatementsCallbackBlock _Nullable) block;
 ...
 *
 * Executing queries returns an @c LVKFMResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 *
 * In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[LVKFMResultSet next]>`) from one record to the other.
 *
 ...
 *
 * @return A @c LVKFMResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 *
 * @see LVKFMResultSet
 * @see [`FMResultSet next`](<[LVKFMResultSet next]>)
 * @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
 *
...
- (LVKFMResultSet * _Nullable) executeQuery :(NSString *) sql, ...;
...
- (LVKFMResultSet * _Nullable) executeQueryWithFormat :(NSString *) format, ... NS_FORMAT_FUNCTION(1, 2);
...
- (LVKFMResultSet * _Nullable) executeQuery :(NSString *) sql withArgumentsInArray :(NSArray *) arguments;
 ...
 * In Swift, this throws errors, as if it were defined as follows:
 *
 * `func executeQuery(sql: String, values: [Any]?) throws  -> LVKFMResultSet!`
 *
 * @param sql The SELECT statement to be performed, with optional `?` placeholders.
 *
 * @param values A @c NSArray  of objects to be used when binding values to the `?` placeholders in the SQL statement.
...
- (LVKFMResultSet * _Nullable) executeQuery :(NSString *) sql values :(NSArray * _Nullable) values error :(NSError * _Nullable __autoreleasing *) error;
...
- (LVKFMResultSet * _Nullable) executeQuery :(NSString *) sql withParameterDictionary :(NSDictionary * _Nullable) arguments;


// Documentation forthcoming.
- (LVKFMResultSet * _Nullable) executeQuery :(NSString *) sql withVAList :(va_list) args;
 ...
 */
- (BOOL) checkpoint :(LVKFMDBCheckpointMode) checkpointMode error :(NSError * _Nullable *) error;
 ...
 * @return @c YES on success, otherwise @c NO .
 */
- (BOOL) checkpoint :(LVKFMDBCheckpointMode) checkpointMode name :(NSString * _Nullable) name error :(NSError * _Nullable *) error;
 ...
 * @return @c YES on success, otherwise @c NO .
 */
- (BOOL) checkpoint :(LVKFMDBCheckpointMode) checkpointMode name :(NSString * _Nullable) name logFrameCount :(int * _Nullable) logFrameCount checkpointCount :(int * _Nullable) checkpointCount error :(NSError * _Nullable *) error;
...
/// The LVKFMDB version number as a string in the form of @c "2.7.8" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[LVKFMDatabase LVKFMDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
...
/** The LVKFMDB version
 *
 * This returns the LVKFMDB as hexadecimal value, e.g., @c 0x0243 for version 2.4.3.
 *
 ...
 *     For this reason, we would recommend using @c LVKFMDBUserVersion  and with @c NSNumericSearch  option, e.g.
 *
 * @code
 * NSComparisonResult result = [[LVKFMDatabase LVKFMDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
 * @endcode
...
+ (SInt32) LVKFMDBVersion __deprecated_msg("Use LVKFMDBUserVersion instead");
 ...
 *  LVKSqliteValueType type = [self.db valueType:argv[0]];
 *  if (type == LVKSqliteValueTypeNull) {
 *      [self.db resultNullInContext:context];
 *       return;
 *  }
 *  if (type != LVKSqliteValueTypeText) {
 ...
 * }];
 *
 * LVKFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
- (LVKSqliteValueType) valueType :(void *) argv;
 ...
 *
 * @code
 * myDB.dateFormat = [LVKFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 * @endcode
 *
 ...
 *
 * @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one LVKFMDB instance and should not be used for other purposes.
 *
 */
 ...
 *
 * @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c LVKFMDatabase:storeableDateFormat .
 *
 * @see hasDateFormatter
 * @see setDateFormat:
 * @see dateFromString:
 ...
 * @see storeableDateFormat:
 *
 * @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to LVKFMDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
 ...
 *
 * This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using LVKFMDB you will not need to interact directly with `LVKFMStatement`, but rather with @c LVKFMDatabase  and @c LVKFMResultSet  only.
 *
 * See also
 *
 * - @c LVKFMDatabase
...
@interface LVKFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
